...
def benchmark_file_based_scanning(
    ...
    if workers == 1:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(check_sidecar, image_path): image_path for image_path in image_paths}

            for future in as_completed(futures):
                exception = future.exception()
                if exception is not None:
                    logger.debug(f"Failed to scan: {exception}")
                    continue
...
def run_benchmark(
    ...
    for path_str in file_results:
        if path_str not in db_results:
            ...
            logger.warning(f"Mismatch for {path_str}: file={file_results[path_str]}, db={db_results[path_str]}")
            mismatches += 1
